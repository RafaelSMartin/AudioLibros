package com.rafaels.audiolibros.zoomSeekBar;
...
public class ZoomSeekBar extends View {
    ...
    // Listener
    private OnValListener escuchador;
    ...
    public ZoomSeekBar(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            val=a.getInteger(R.styleable.ZoomSeekBar_val,1);
...
//        yIni = getPaddingTop();
        // Centro la altura de la vista y control de error
    ...
    protected void onDraw(Canvas canvas) {
        ...
        palancaRect.set(x - anchoPalanca / 2, y, x + 3 * anchoPalanca / 2,
                y + altoPalanca);
        // Dibujamos Escala
        int v = escalaIni;
        while (v <= escalaMax) {
...
//                    val = ponDentroRango(val_0, escalaMin, escalaMax);
    ...
    public void setVal(int val) {
        if (valMin <= val && val <= valMax) {
            ...
            invalidate();
            this.escuchador.onChangeVal(val);
        }
    }

    public void setValNoEvent(int val){
        if (valMin <= val && val <= valMax) {
            this.val = val;
            escalaMin = Math.min(escalaMin, val);
            escalaMax = Math.max(escalaMax, val);
    ...
    public void setOnValListener(OnValListener escuchador){
